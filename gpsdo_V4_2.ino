...
/*
  ...
  This software is ONLY for 328 based Arduino CPU's !!
  With modulo 50000
  ...
  if (Serial.available() > 0)       //process if something is there
    {
      ...
      case Q:      
        ...
        case u:                 // correction factor for the room temperature sensor
        case U:                 // input value x 10 to add decimals, add 100 for negative numbers
                                // the correction numbers will start at 0.0 at bootup and will be cumulative
          zz = Serial.parseInt();
          if (zz > 100)
            { // negative numbers
              Serial.print(F("u "));
              roomTempCorr = -(zz-100)/10.0;
              Serial.println(roomTempCorr,1);
            }
          if (zz < 100)
            { // positive numbers
              Serial.print(F("u "));
              roomTempCorr = zz/10.0 ; 
              Serial.println(roomTempCorr,1); 
            }               
          break;
  ...
  if (lessInfoDisplayed == false)
    {
      ...
      // added IIR Low Pass Filter for room temperature sensor readings
      tempADC3_IIR = tempADC3_IIR + ((tempADC3 - tempADC3_IIR) / IIR_Temp_Filter_Weight);
      Serial.print(temperature_to_C(tempADC3_IIR,temperature_Sensor_Type%10) + roomTempCorr, 1);
      Serial.print("\t");  
...
{
  Serial.print(F(" paulv Version  "));
  Serial.print(FW_VERSION); 
  Serial.print(F(" firmware PCB Version "));
  Serial.print(HW_VERSION); // *** paulv: added my firmware and hw version indication 
...
{ 
  ...
  //Serial.print(F("qErr"));  // *** paulv: added qErr
  //Serial.print("\t");
  //Serial.print(F("nsRAW")); // applied result
  //Serial.print("\t");
...
void setup() 
{
  ...
  // Set the "16bit DAC" 
  mydac.setValue(dacValueOut); // *** paulv send the 16-bit value over SPI to the DAC
